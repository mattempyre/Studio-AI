 ...
 * STORY 4.3: Video Prompt Generation (Wan 2.2 format)
 *
 * Provides endpoints for generating image and video prompts from script sentences.
 */
...
// Validation schema for generate-video-prompts request
const generateVideoPromptsSchema = z.object({
  sentenceIds: z.array(z.string()).optional(),
  force: z.boolean().optional().default(false),
  async: z.boolean().optional().default(true), // If true, use Inngest job queue
});
...
/**
 * POST /api/v1/projects/:id/generate-video-prompts
 ...
 * - data: { total, generated, prompts[] }
 */
promptsRouter.post('/:id/generate-video-prompts', async (req: Request, res: Response, next) => {
  try {
    const projectId = req.params.id as string;

    // Validate request body
    const parsed = generateVideoPromptsSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({
        success: false,
        error: {
    ...
    if (useAsync) {
      ...
      return res.status(202).json({
        success: true,
        message: 'Video prompt generation queued',
        eventId: event.ids[0],
      });
    }
    ...
    if (!result.success) {
      return res.status(result.errors?.includes('Project not found') ? 404 : 500).json({
        ...
        error: {
          code: result.errors?.includes('Project not found') ? 'NOT_FOUND' : 'GENERATION_ERROR',
          message: result.errors?.join('; ') || 'Failed to generate video prompts',
        },
      });
    }
...
/**
 * GET /api/v1/projects/:id/video-prompts
 * Get all video prompts for a project's sentences.
 * Useful for checking which sentences have video prompts generated.
 */
promptsRouter.get('/:id/video-prompts', async (req: Request, res: Response, next) => {
  try {
    const projectId = req.params.id as string;
    ...
    const allSentences = await db
      .select({
        ...
        text: sentences.text,
        imagePrompt: sentences.imagePrompt,
        videoPrompt: sentences.videoPrompt,
        sectionId: sentences.sectionId,
        order: sentences.order,
      })
      .from(sentences)
    ...
    const withPrompts = allSentences.filter(s => s.videoPrompt && s.videoPrompt.trim() !== '').length;
    ...
    res.json({
      ...
      data: {
        ...
        sentences: allSentences.map(s => ({
          id: s.id,
          text: s.text.substring(0, 100) + (s.text.length > 100 ? '...' : ''),
          hasVideoPrompt: !!(s.videoPrompt && s.videoPrompt.trim() !== ''),
          videoPrompt: s.videoPrompt,
          imagePrompt: s.imagePrompt, // Include for reference
        })),
      },
    });
  } catch (error) {
